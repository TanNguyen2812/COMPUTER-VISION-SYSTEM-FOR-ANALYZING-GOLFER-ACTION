...
class FG_TFormer(nn.Module):
    def __init__(self, channel, head=4, kernel_size=3, dilation=1, p_dropout=0.1):
        super(FG_TFormer, self).__init__()
        assert channel % head == 0
        self.dk = int(channel // head)
        self.head = head
        self.attention = ScaleDotProductAttention()
        pad = (kernel_size + (kernel_size - 1) * (dilation - 1) - 1) // 2
        self.Q_conv = nn.Conv2d(channel, channel, kernel_size=(kernel_size, 1), padding=(pad, 0),
                                stride=(1, 1), dilation=(dilation, 1))

        self.K_conv = nn.Conv2d(channel, channel, kernel_size=(kernel_size, 1), padding=(pad, 0),
                                stride=(1, 1), dilation=(dilation, 1))

        self.V_conv = nn.Conv2d(channel, channel, kernel_size=(kernel_size, 1), padding=(pad, 0),
                                stride=(1, 1), dilation=(dilation, 1))
        self.lin = nn.Conv2d(channel, channel, kernel_size=1)
    ...
    def forward(self, x, mask=None):
        ...
        # N C T V
        V = self.V_conv(x)  # N, C, T, V
        V = V.permute(0, 3, 2, 1).contiguous()  # N, V, T, C

        Q = Q.view(N, J, T, self.head, self.dk)  # N, V, T, n_head, dk
        K = K.view(N, J, T, self.head, self.dk)
        V = V.view(N, J, T, self.head, self.dk)

        Q = Q.transpose(2, 3).contiguous()  # N, V, n_head, T, dk
        ...
        output = output.transpose(2, 3).contiguous().view(N, J, T, C)

        # x = x.permute(0, 3, 2, 1).contiguous()
        output = output.permute(0, 3, 2, 1).contiguous()
        output = self.lin(output)
        return self.dropout(self.bn(output))
    ...
    def __init__(self,
        ...
        for i, cfg in enumerate(ms_cfg):
            ...
            if cfg[0] == 'max':
                branches.append(
                    nn.Sequential(
                        nn.Conv2d(in_channels, branch_c, kernel_size=1), LayerNorm(branch_c), self.act,
                        nn.MaxPool2d(kernel_size=(cfg[1], 1), stride=(stride, 1), padding=(1, 0))))
                continue
            assert isinstance(cfg[0], int) and isinstance(cfg[1], int)
            branch = nn.ModuleList([nn.Conv2d(in_channels, branch_c, kernel_size=1), LayerNorm(branch_c), self.act,
        ...
        self.transform = nn.Sequential(
            LayerNorm(tin_channels), self.act, nn.Conv2d(tin_channels, out_channels, kernel_size=1))
...
class MLPHead(nn.Module):
    def __init__(self, projection_hidden_size=128):
        super().__init__()

        self.embedding_size = projection_hidden_size
        self.net = nn.Sequential(nn.Linear(self.embedding_size, projection_hidden_size),
                                 nn.LayerNorm(projection_hidden_size),
                                 nn.ReLU(True),
...
class ST_GCN(nn.Module):
    ...
    def __init__(self, in_channels, embedding_size):
        ...
        self.data_bn1 = LayerNorm(in_channels)
        self.data_bn2 = LayerNorm(in_channels)
        self.data_bn3 = LayerNorm(in_channels)
        # self.data_bn = nn.LayerNorm(in_channels * A.shape[1])

        self.l1_s1 = st_gcn_unit(in_channels, 64, A, ms_cfg=[(9, 1), (3, 2), (3, 2 ** (8 - 1))])
        self.l2_s1 = st_gcn_unit(64, 64, A, ms_cfg=[(9, 1), (3, 2 ** 2), (3, 2 ** (8 - 2))])
        self.l3_s1 = st_gcn_unit(64, 30, A, ms_cfg=[(9, 1), (3, 2 ** 3), (3, 2 ** (8 - 3))])

        self.l1_s2 = st_gcn_unit(in_channels, 64, A, ms_cfg=[(9, 1), (3, 2), (3, 2 ** (8 - 1))])
        self.l2_s2 = st_gcn_unit(64, 64, A, ms_cfg=[(9, 1), (3, 2 ** 2), (3, 2 ** (8 - 2))])
        self.l3_s2 = st_gcn_unit(64, 30, A, ms_cfg=[(9, 1), (3, 2 ** 3), (3, 2 ** (8 - 3))])

        self.l1_s3 = st_gcn_unit(in_channels, 64, A, ms_cfg=[(9, 1), (3, 2), (3, 2 ** (8 - 1))])
        self.l2_s3 = st_gcn_unit(64, 64, A, ms_cfg=[(9, 1), (3, 2 ** 2), (3, 2 ** (8 - 2))])
        self.l3_s3 = st_gcn_unit(64, 30, A, ms_cfg=[(9, 1), (3, 2 ** 3), (3, 2 ** (8 - 3))])

        # self.l3 = st_gcn_unit(90, 100, A, ms_cfg=[(9,1), (3, 2**3),(3, 2**(8-3))])
        self.l4 = st_gcn_unit(90, 128, A, ms_cfg=[(9, 1), (3, 2 ** 4), (3, 2 ** (8 - 4))])
        self.l5 = st_gcn_unit(128, 128, A, ms_cfg=[(9, 1), (3, 2 ** 5), (3, 2 ** (8 - 5))])
        self.l6 = st_gcn_unit(128, 128, A, ms_cfg=[(9, 1), (3, 2 ** 6), (3, 2 ** (8 - 6))])
        self.l7 = st_gcn_unit(128, 128, A, ms_cfg=[(9, 1), (3, 2 ** 7), (3, 2 ** (8 - 7))])
        self.l8 = st_gcn_unit(128, 128, A, ms_cfg=[(9, 1), (3, 2 ** 8), (3, 2 ** (8 - 8))])
        ...
        self.protection_head = MLPHead(embedding_size)
        self.protection = False

    def forward(self, j, b, v, mask=None):
        if mask is not None:
            mask = mask.unsqueeze(1)
        N, T, V, C = j.size()
        j = j.permute(0, 3, 1, 2).contiguous()
